...
function RippleGate(options) {
  ...
  this.check = function(req, res, next) {
    if (req.session.rgExpire == null) {
      ...
      remote.connect(function() {
        ...
        request.callback(function(err, data) {
          for (var i = 0; i < data.transactions.length; i++) {
            if (data.transactions[i].tx.DestinationTag == req.session.rgid && data.transactions[i].meta.TransactionResult == 'tesSUCCESS') {
      ...
      console.log(expire.toString() + '  compare to  ' + now.toString());
  ...
  this.ensureID = function(req, res, next) {
    if (!req.session.hasOwnProperty('rgid')) {
      req.session.rgid = Math.random()*4294967295;
      req.session.rgExpire == null;
    }
    return next();
  }
